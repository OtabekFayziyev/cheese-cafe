...
export default function Home() {
  ...
  // Display name from Telegram or store
  ...
  const endDrag = () => {
    ...
    if (Math.abs(delta) > 45) {
      // delta < 0 = swipe left = next slide
      // delta > 0 = swipe right = prev slide
      if (delta < 0) {
  ...
  return (
    <AppShell>
      <Page>
        ...
        {activeOrder && (
          <div className={styles.trackingBanner} onClick={() => navigate('/user')}>
            <div className={styles.trackingLeft}>
              ...
              <div>
                ...
                <div className={styles.trackingSub}>Kuryeringiz: Jasur K. · ~12 daqiqa</div>
              </div>
            </div>
        ...
        {/* Pizza builder promo */}
        <div className={styles.builderBanner} onClick={() => { haptic.light(); navigate('/user/pizza-builder') }}>
          <div>
        ...
        <SectionHeader title="Menyu" action={
          <button onClick={() => setActiveCat('all')} style={{background:'none',border:'none',cursor:'pointer',fontSize:13,fontWeight:700,color:'var(--yellow-dark)'}}>
            Barchasi →
          </button>
        } />
